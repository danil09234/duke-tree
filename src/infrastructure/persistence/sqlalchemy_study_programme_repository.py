...
class SQLAlchemyStudyProgrammeRepository(Savable[TukeStudyProgramme]):
    def __init__(
            self,
            session_maker: async_sessionmaker[AsyncSession],
            study_programme_mapper: EntityMapper[TukeStudyProgramme, StudyProgrammeORM]
    ):
        self._session_maker = session_maker
        self._study_programme_mapper = study_programme_mapper

    async def save(self, study_programme: TukeStudyProgramme) -> None:
        model = await self._study_programme_mapper.from_entity(study_programme)
        async with self._session_maker() as session:
            session.add(model)
            await session.commit()

    async def save_multiple(self, study_programmes: list[TukeStudyProgramme]) -> None:
        models = [
